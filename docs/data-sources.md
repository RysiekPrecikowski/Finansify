...
| GPW quotes                      | `gpw`'s own unofficial `chart-json.php` (`gpw.pl`), Yahoo `.WA` in the chain as fallback | `GET chart-json.php?req=[{"isin":"<ISIN>","mode":"<window>"}]`, keyed by ISIN, no auth — but a WAF resets the connection outright for anything that doesn't carry a browser-like `Referer`/`Sec-Fetch-*` set (confirmed live, ADR 0022). Daily OHLCV only, equities and ETFs (`fund` is `bankier`'s, `bond` is deferred to ADR 0023). `mode: 'ARCH'` returns an instrument's entire history in one request (confirmed to 1999 for a real listing) — used only for a `from` older than the widest fixed window, so a routine refresh stays small. Stooq, the previously planned fallback, is confirmed dead (proof-of-work anti-bot challenge, not circumvented). |
