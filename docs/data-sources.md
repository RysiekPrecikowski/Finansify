...
| Equity, ETF (GPW)               | `gpw` (`chart-json.php`), Yahoo `.WA` as fallback | Continuously, 09:00–17:00 CET                                     | 15 min                               |
| Fund (TFI/PPK)                  | `bankier` (`bankier-fund-chart-data`)             | Per fund's own valuation cadence — daily or weekly, never assumed | 15 min                               |
...
| GPW quotes                      | `gpw`'s own unofficial `chart-json.php` (`gpw.pl`), Yahoo `.WA` in the chain as fallback | `GET chart-json.php?req=[{"isin":"<ISIN>","mode":"<window>"}]`, keyed by ISIN, no auth — but a WAF resets the connection outright for anything that doesn't carry a browser-like `Referer`/`Sec-Fetch-*` set (confirmed live, ADR 0022). Daily OHLCV, equities/ETFs and Catalyst-listed bonds (`catalyst_bond`) alike — a Catalyst bar is money per 100 nominal, same as any other close (ADR 0023); `fund` (TFI/PPK) stays `bankier`'s, retail treasury `bond` is never quoted at all (it accrues, ADR 0011). `mode: 'ARCH'` returns an instrument's entire history in one request (confirmed to 1999 for a real listing) — used only for a `from` older than the widest fixed window, so a routine refresh stays small. Stooq, the previously planned fallback, is confirmed dead (proof-of-work anti-bot challenge, not circumvented).                                                                                                                                                                                                                                                       |
| Catalyst bond nominal value     | `gpwcatalyst.pl`'s instrument page                                                       | `GET o-instrumentach-instrument?nazwa=<ticker>` — keyed by the Catalyst **ticker** (`GHE0128`), not ISIN; a different identifier from `chart-json.php`'s, for a different GPW site (ADR 0023). Structured table row (`Wartość nominalna (PLN): 100,00`), regexed off the raw markup rather than flattened text so the label stays pinned to its own value. An unrecognised ticker renders "Brak danych" with no matching row — the regex miss is `null`, no separate not-found branch. Fixed for the issue's life; cached in `catalyst_bond_terms`, resolved once per symbol (ADR 0011's pattern).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| TFI/PPK fund quotes             | bankier.pl's public chart API                                                            | `GET api.bankier.pl/quotes/public/bankier-tfi/bankier-fund-chart-data?symbol=<symbol>&range=<window>`, keyed by bankier's own fund symbol (`AGF04`), unrelated to the fund's ISIN — a third identifier alongside GPW's ISIN and Catalyst ticker. **No auth, no WAF at all** (confirmed live: a bare `fetch(url)` with zero headers answers `200`) — the retry/backoff this adapter still does covers only a genuine transport or server failure, not an unmapped symbol. One quote per unit per valuation day; some funds value weekly, not daily — the series carries exactly the days bankier published, never resampled. `range=max` returns the fund's entire tracked history in one response (confirmed live: 15 years, ~3,900 points); a routine refresh asks the smallest fixed range (`1m`…`5y`) that covers `from`. The response's own `profile_data.currency` is checked against the instrument's stored currency and the fetch refused on a mismatch — data GPW's endpoint never provides to check against. An unrecognised symbol answers `200` with an empty series, not an error. |
| TFI/PPK instrument search       | bankier.pl's instrument-search API                                                       | `GET api.bankier.pl/quotes/public/instrument-search/?bankier=true&query=<text>` — one JSON response keyed by asset class (`funds_tfi`, `ppk`, `companies`, `bonds`, …), each entry carrying the same `kod`/`code` symbol the chart endpoint above answers to (ADR 0024). No auth, same as the chart endpoint. `bonds` in this response stayed empty for every Catalyst issuer tried (Ghelamco, Kruk, PCC, PKO) — this is not where Catalyst bonds live.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Catalyst bond search            | `gpwcatalyst.pl`'s corporate-bond listing page                                           | No search API on this site at all (checked). `GET notowania-obligacji-obligacje-korporacyjne` returns the whole corporate-bond board server-rendered as one HTML table (~1.5 MB, confirmed live: ~640 rows), issuer names `rowspan`-grouped over every bond from that issuer — filtered in memory (ADR 0024). Municipal, covered, cooperative and convertible bonds sit on their own listing pages and are a deliberate scope cut. `o-instrumentach-instrument?nazwa=<ticker>` (the same page ADR 0023's nominal comes from) also carries the bond's ISIN (`id="isin"` hidden input) and issuer name (`Nazwa emitenta` row) — everything a selection flow needs in the one fetch it was already making.                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Bond daily interest tables      | Bank Pekao, `pekao.com.pl/.rest/gb-interest-tables/…`                                    | **Plain JSON REST, no auth, no WAF** — the same official tables, from a second emission agent, and now the **primary source of a bond's value** (ADR 0019). Covers ROR, DOR, TOS, COI, EDO; OTS, ROS and ROD publish none and stay on `accrueBond`. Fetched lazily per held series and cached in `bond_interest_tables`; a stored table is final, since a period's whole grid is published as soon as its rate is known. Keyed `{period}-{purchase day}` with the day taking only 1, 29, 30 and 31 — which covers every purchase day, because a daily value depends on (rate, period length, days elapsed) and not on the calendar. ADR 0019.                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
